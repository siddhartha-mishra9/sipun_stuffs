...
void loop()
{
  ...
  Serial.println(sensorPastAverage); 
...
{
  if(sensorPastAverage != currentThreshold)
  {
    return true;
  }
  else
...
{
  pastValuesAverage[idx]=sensorValue;
  totalsmooth += totalsmoothpastValuesAverage[idx];
  idx++;
  if(idx == 1000)
  {
    idx=0;
  }
  sensorPastAverage=(totalsmooth / 1000);
